...
logging.basicConfig(handlers=[RotatingFileHandler(os.path.join(sys.path[0],log_file), maxBytes=10240000, backupCount=5)], format='%(asctime)s.%(msecs)03d %(levelname)-5s %(filename)s [%(processName)s|%(threadName)s] %(message)s', datefmt='%Y-%m-%d %H:%M:%S', level=logging.DEBUG)
...
if __name__ == "__main__":
   ...
   if len(AGENT_ID or '') != 0:
       if AGENT_ID == "ffffffff-ffff-ffff-ffff-ffffffffffff" or AGENT_ID == "00000000-0000-0000-0000-000000000000":
          ...
          print("\nThe hard configured ID '"+AGENT_ID+"' is reserved for internal use. Aborting !\n")
          sys.exit(3)
   ...
   print("\nSIAAS Agent v"+SIAAS_VERSION+" starting ["+AGENT_ID+"]\n\nLogging to: "+os.path.join(sys.path[0],log_file)+"\n")
