...
def main(version="N/A"):
    ...
    try:
       ...
       bt = datetime.utcfromtimestamp(boot_time_timestamp).strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    try:
       agent["platform"]["cpu"]={}
       agent["platform"]["cpu"]["physical_cores"]=psutil.cpu_count(logical=False)
       agent["platform"]["cpu"]["total_cores"]=psutil.cpu_count(logical=True)
       cpu_freq = psutil.cpu_freq()
       agent["platform"]["cpu"]["current_freq"]=str(cpu_freq.current)+ " MHz"
    ...
    try:
       ...
       agent["platform"]["memory"]["available"]=siaas_aux.get_size(svmem.available)
       ...
       agent["platform"]["memory"]["swap"]["total"]=siaas_aux.get_size(swap.total)
       agent["platform"]["memory"]["swap"]["free"]=siaas_aux.get_size(swap.free)
       agent["platform"]["memory"]["swap"]["used"]=siaas_aux.get_size(swap.used)
       agent["platform"]["memory"]["swap"]["present"]=siaas_aux.get_size(swap.total)
       agent["platform"]["memory"]["swap"]={}
    except:
       ...
       for partition in partitions:
           if partition.device.startswith("/dev/loop"):
               ...
               try:
                  ...
                  agent["platform"]["io"]["volumes"][partition.device]["usage"]["total"]=siaas_aux.get_size(partition_usage.total)
                  agent["platform"]["io"]["volumes"][partition.device]["usage"]["used"]=siaas_aux.get_size(partition_usage.used)
                  agent["platform"]["io"]["volumes"][partition.device]["usage"]["free"]=siaas_aux.get_size(partition_usage.free)
                  agent["platform"]["io"]["volumes"][partition.device]["usage"]["percentage"]=str(partition_usage.percent)+" %"
               except:
                  pass
       disk_io = psutil.disk_io_counters()
       agent["platform"]["io"]["total_read"]=siaas_aux.get_size(disk_io.read_bytes)
       agent["platform"]["io"]["total_write"]=siaas_aux.get_size(disk_io.write_bytes)
    except:
    ...
    try:
           ...
           for address in interface_addresses:
               if str(address.family) == 'AddressFamily.AF_INET':
                   if interface_name not in address.address:
                      mask_prefix=ipaddress.IPv4Network("0.0.0.0/"+address.netmask).prefixlen
                      list_addr_mask.append(address.address+"/"+str(mask_prefix))
       ...
       agent["platform"]["network"]["total_sent"]=siaas_aux.get_size(net_io.bytes_sent)
       agent["platform"]["network"]["total_received"]=siaas_aux.get_size(net_io.bytes_recv)
    except:
       logger.warning("Couldnt get network information and statistics. Ignoring.")
